...
export async function exportInvestigators(investigators: Investigator[]) {
  const data = investigators?.map((o, i) => {
    return {
...
export async function exportSignins(accounts: Account[]) {
  const data = accounts?.map((o, i) => {
    return {
      'Email address': o.emailAddress,
      Role: o.role,
      'Signed in':
        o.sessionAuthenticatedTimestamp == null
          ? null
          : moment(o.sessionAuthenticatedTimestamp).format('MM/DD/YYYY hh:mmA'),
      'Last active':
        o.lastActiveTimestamp == null
          ? null
          : moment(o.lastActiveTimestamp).format('MM/DD/YYYY hh:mmA'),
    }
  })
...
export async function exportJobs(jobs: Job[]) {
  const data = jobs?.map((o, i) => {
    return {
      ...
      Interval: o.interval,
      'Next event':
        o.nextEvent == null ? null : moment(o.nextEvent).format('MM/DD/YYYY hh:mmA'),
    }
  })
...
export async function exportClaims(claims: Claim[]) {
  const data = claims?.map((o, i) => {
    return {
      ...
      'Event date':
        o.eventDate == null ? null : moment(o.eventDate).format('MM/DD/YYYY hh:mmA'),
