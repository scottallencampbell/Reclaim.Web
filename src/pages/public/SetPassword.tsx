  ...
  return (
    <UnauthenticatedLayout id="setpassword" title={title} message={message} errorMessage={errorMessage} reversed={true}>
      <form className={(errorMessage.length > 0 ? "form-error" : "") + (isSuccessful ? "hidden" : "")} onSubmit={handleSubmit}>                        
        <div className="mb-3">
          <TextInput type="password" label="Password" name="password" required={true} value={password} onChange={(value:string) => setPassword(value)}></TextInput>
        </div>                       
        <div className="mb-3">
          <TextInput type="password" label="Retype password" name="password2" required={true} value={password2} onChange={(value:string) => setPassword2(value)}></TextInput>
        </div>                       
