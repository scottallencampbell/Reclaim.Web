...
const Register = () => {
  ...
  useEffect(() => {    
    // renable this if you want investigators to create passwords upon registration, rather than after email confirmation
  ...
  useEffect(() => {    
      setIsSubmitButtonEnabled(
         firstName.length > 0 && lastName.length > 0 && emailAddress.length > 0
          ...
          && new RegExp(emailAddressRegex).test(emailAddress) && new RegExp(telephoneRegex).test(telephone)    
        );
   }, [isPasswordAllowed, firstName, lastName, emailAddress, telephone, password, password2 ]);
   ...
   const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await attemptRegister(async () => { await register(firstName, lastName, emailAddress, telephone, password) });       
  }
        ...
        <form className={(errorMessage.length > 0 ? "form-error" : "")} onSubmit={handleSubmit}>
          <div className="row">
            <div className="col-lg-6">
              <TextInput type="text" label="First name" name="first-name" value={firstName} required={true} onChange={(value: string) => setFirstName(value)}></TextInput>
            </div>
            <div className="col-lg-6">
              <TextInput type="text" label="Last name" name="last-name" value={lastName} required={true} onChange={(value: string) => setLastName(value)}></TextInput>
            </div>
          </div>
          <div className="mb-3">
            <TextInput type="text" label="Email address" name="email-address" value={emailAddress} required={true} onChange={(value: string) => setEmailAddress(value)}></TextInput>
          </div>
          <div className="mb-3">
            <TextInput type="telephone" label="Telephone" name="telephone" value={telephone} required={true} onChange={(value: string) => setTelephone(value)}></TextInput>
          </div>
          {!isPasswordAllowed ? <></> :
            <>
              <div className="mb-3">
                <TextInput type="password" label="Password" name="password" value={password} required={true} onChange={(value: string) => setPassword(value)}></TextInput>
              </div>
              <div className="mb-3">
                <TextInput type="password" label="Retype password" name="password2" value={password2} required={true} onChange={(value: string) => setPassword2(value)}></TextInput>
              </div>
            </>
          }
