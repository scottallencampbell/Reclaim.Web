...
const SignIn = () => {
  ...
  const handleGoogleSubmit = async (credentialResponse: any) => {
    ...
    await signIn(async () => {
      await authorizeGoogle(credentialResponse.credential, nonce)
    })
  }
  ...
  const validate = () => {
    ...
    if (emailAddresses.length === 2) {
      if (
        new RegExp(emailAddressRegex).test(emailAddresses[0]) &&
        new RegExp(emailAddressRegex).test(emailAddresses[1])
      ) {
        isValid = true
      }
