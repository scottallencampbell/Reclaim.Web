...
const Table = ({ children, id, type, keyField, columns, sourceData, isPropertyBarVisible, initialSortColumn, initialSortOrder, onSearchTermsChange, onRowClick }: ITable) => {
  ...
  const sort = (newSortColumn: string, currentSortOrder: string, type: string) => {
    ...
    if (newSortColumn) {
      const sorted = [...data].sort((a, b) => {
        if (a[newSortColumn] === null) return 1;
        if (b[newSortColumn] === null) return -1;
        if (a[newSortColumn] === null && b[newSortColumn] === null) return 0;
        ...
        switch (type) {
          ...
          default:
            return (
              a[newSortColumn].toString().localeCompare(b[newSortColumn].toString(), "en", {
                numeric: true,
              }) * (newSortOrder === "asc" ? 1 : -1)
            );
        ...
        <div className={`table${isHoverable ? " is-hoverable" : ""}`}>
          ...
          <table id={id}>
            ...
            <tbody onMouseMove={(ev) => handleMouseMove(ev)}>
              {data.map((item: any) => {
                return (
                  <tr id={`row-${item[keyField]}`} key={item[keyField]} className={item.isHidden ? "hidden" : ""} onClick={() => handleRowClick(item)}>
                    {columns.map(({ accessor, type }) => {
                      ...
                      if (value) {
                        switch (type) {
                          ...
                          case "hash":
                            cell = value.length > 8 ? value.substring(0, 4) + " ... " + value.substring(value.length - 4) : value;
                            break;
                          ...
                          case "thumbnail":
                            if (value != null && value != "")
                              tag = <div style={{ backgroundImage: `url(${value})` }} className="thumbnail"></div>;
                            break;
